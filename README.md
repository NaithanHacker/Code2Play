# Code2Play
Turning Code into Fun - A Game Development repository where ideas becomes playable experiences.

CodeQuest is an open-world, game-based learning platform designed to make programming interactive, engaging, and fun.
Inspired by games like GTA V and Minecraft, this project allows users to learn programming languages by completing real coding missions inside a virtual world.
Instead of traditional tutorials, users learn by playing, exploring, and solving problems.
🧩 Problem Statement
Learning programming is often:
Boring and text-heavy
Confusing for beginners
Lacking real-world practice
Demotivating, leading to high dropout rates
Existing platforms fail to provide an immersive and interactive learning experience.
💡 Our Solution
CodeQuest transforms programming education into a game-driven experience where:
Users log in and enter an open-world environment
Different zones represent different programming languages
Users choose their learning path and complete level-based challenges
Progress is tracked using XP, rewards, and achievements
🎮 Game Features
🔐 User Authentication
Secure login and signup
Progress and achievements saved
🌍 Open-World Environment
Virtual city / lab-based map
Dedicated zones for:
Python
Java
C / C++
Web Development
🧠 Level-Based Learning
Beginner to advanced levels
Real coding challenges
Debugging missions and mini-projects
🏆 Gamification
XP points and levels
Badges and achievements
Unlockable areas
Leaderboard system
🛠️ Tech Stack
Game Engine: Unity / Unreal Engine / Three.js
Frontend: HTML, CSS, JavaScript
Backend: Node.js / Firebase
Authentication: Firebase Auth
Database: Firestore / MongoDB
Code Execution: Secure sandboxed compiler APIs
🚀 How It Solves the Problem
Challenge
Solution
Boring learning
Game-based missions
Language confusion
Clear language zones
Low motivation
Rewards & XP system
Lack of practice
Real coding challenges
Dropouts
Fun & immersive gameplay
📈 Future Scope
Multiplayer coding battles
AI-based personalized difficulty
More programming languages
Mobile and VR support
Integration with EdTech platforms
🧪 Project Status
🚧 Currently under development
This project is a hackathon prototype, and features may evolve.
🤝 Team
👩‍💻 Bhumika Yadav – Developer & Ideation
👥 Naithan Kant Jena – Game Design & Development
⭐ One-Line Pitch
An open-world game where players don’t just play — they learn programming by completing real coding missions.
📜 License
This project is licensed under the MIT License.
